...
namespace NetCoreServer
{
	...
	public class Buffer
	{
		...
		/// <summary>
		/// Get a span of bytes from the current buffer, safe against <see cref="Shift(long)"/> operations
		/// </summary>
		public Span<byte> AsSpanSafe()
		{
			int remaining = (int)(_data.Length - _offset);
			if (remaining < 0)
				remaining = 0;
			return new Span<byte>(_data, (int)_offset, (int)Math.Max(0, _size - _offset));
		}
