...
namespace NetCoreServer
{
		...
		where SESSION_TYPE : BaseSession, new()
	{
		...
		protected virtual Socket CreateSocket()
		{
			...
			// Create the socket using InterNetworkV6 (IPv6)
			// SocketType.Stream is for TCP; use SocketType.Dgram for UDP
			Socket l_socket = new Socket(AddressFamily.InterNetworkV6, SocketType.Stream, ProtocolType.Tcp);
			return l_socket;
		}
		...
		public virtual bool Start()
		{
			...
			//commented due to ipv6 dual protocol addition 
			//_acceptorSocket.SetSocketOption(SocketOptionLevel.Socket, SocketOptionName.ReuseAddress, OptionReuseAddress);
			// Apply the option: exclusive address use
			//commented change due to ipv6 dual protocol addition
			//_acceptorSocket.SetSocketOption(SocketOptionLevel.Socket, SocketOptionName.ExclusiveAddressUse, OptionExclusiveAddressUse);

			// Ensure the socket is dual-stack (can accept both IPv4 and IPv6 connections)
			// If the platform default is true, this line is necessary to disable it.
			// In many modern .NET environments on Linux, the default behavior might already be dual-stack.
			_acceptorSocket.SetSocketOption(SocketOptionLevel.IPv6, SocketOptionName.IPv6Only, false);
			...
			// setting endpoint to accept any ipv6 and ipv4 mapped to ipv6 client to port 
